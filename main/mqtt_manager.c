...
/* Topics are built once at init from DEVICE_NAME */
...
static void heartbeat_cb(void *arg) {
    ...
    char buf[160];
    snprintf(buf, sizeof(buf), "{\"uptime\":%lu,\"free_heap\":%lu,\"device\":\"%s\"}", (unsigned long)uptime_s,
             (unsigned long)free_heap, DEVICE_NAME);
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
    ...
    switch ((esp_mqtt_event_id_t)event_id) {
        ...
        case MQTT_EVENT_CONNECTED:
            ...
            mqtt_manager_publish(s_topic_status, "{\"event\":\"online\",\"device\":\"" DEVICE_NAME "\"}");
            break;
